...
#model_name = 'BPL_YEAST.Fedbatch' 
#application_file = 'BPL_YEAST.mo'
...
if platform.system() == 'Windows':
   ...
   fmu_model ='BPL_YEAST_AIR_Fedbatch_DOcontrol_linux_jm_cs.fmu'        
   model = load_fmu(fmu_model, log_level=0)
   opts = model.simulate_options()
   opts['silent_mode'] = True
else:    
...
def newplot(title='Yeast fedbatch cultivation', plotType='TimeSeries'):
   ...
   if plotType in ['Overview']:
      ...
      diagrams.append("ax41.plot(t,sim_res['DO_setpoint.y'],color='c',linestyle='--')")
